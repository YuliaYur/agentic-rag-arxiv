...
def build_graph(retriever, llm, config: AgentConfig | None = None):
    cfg = config or AgentConfig()
    nodes = AgentNodes(retriever, llm, cfg)
    ...
    g.add_conditional_edges("cite_critic", route_after_critic, {"generate": "generate", "end": END})
    return g.compile()
...
def build_agent(config: AgentConfig | None = None):
    ...
    return build_graph(build_retriever(), LLMClient(), config or AgentConfig())
