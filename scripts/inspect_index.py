...
def main() -> int:
    ...
    offset = None
    while True:
        points, offset = client.scroll(cfg.qdrant.collection, limit=1000, offset=offset,
                                        with_payload=["n_tokens"], with_vectors=False)
    ...
    print(f"chunk tokens (model tokenizer): min={tokens[0]} median={tokens[n // 2]} "
    ...
    hits = client.query_points(cfg.qdrant.collection, query=qvec, limit=args.top,
                               with_payload=True).points

    print(f'Query: "{args.query}"\n' + "-" * 78)
    for h in hits:
        pl = h.payload
        pages = f"p.{pl['page']}" if pl["page"] == pl["page_end"] else f"p.{pl['page']}-{pl['page_end']}"
